...
import { ArrowLeft, MapPin, Settings2, X } from 'lucide-react';
import { PageHeader } from '@/components/app-shell/PageHeader';
import { EmptyState } from '@/components/ui/EmptyState';
import { MediaTile } from '@/components/archive/MediaTile';
import { getPlace } from '@/lib/archive/places';
import {
  deletePlaceAction,
  removePlaceFromItemAction,
  renamePlaceAction,
} from '@/lib/archive/places-actions';

export async function generateMetadata({ params }: { params: { id: string } }) {
  const place = await getPlace(params.id);
  return { title: place?.name ?? 'Plaats' };
}
...
export default async function PlaceDetailPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div>
      ...
      <PageHeader title={place.name} subtitle={t('count', { count: place.items.length })} />
      ...
      <details className="mb-6">
        ...
        <div className="mt-3 space-y-3">
          <form action={renamePlaceAction} className="flex max-w-md gap-2">
      ...
      {place.items.length === 0 ? (
        <EmptyState
          ...
          title={t('emptyPlaceTitle')}
          description={t('emptyPlaceBody')}
        />
      ) : (
        <ul className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
          {place.items.map((item) => (
            <li key={item.id} className="group relative">
              ...
              <form
                action={removePlaceFromItemAction}
              ...
              >
                ...
                <button
                  type="submit"
                  aria-label={t('removeFromPlace')}
                  title={t('removeFromPlace')}
